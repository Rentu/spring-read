...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// Invoke factory processors registered as beans in the context. 将工厂处理器注册为bean到容器中
	 ...
	 * 准备容器以进行刷新，设置其启动日期和活动标志以及执行属性源的任何初始化。
	 */
	protected void prepareRefresh() {
		...
		// 这个 earlyApplicationListeners 是干啥的？ TODO
		if (this.earlyApplicationListeners == null) {
			this.earlyApplicationListeners = new LinkedHashSet<>(this.applicationListeners);
		}
		else {
			// Reset local application listeners to pre-refresh state. 重置local application listeners 为  预刷新状态
		...
		// 收集 初期应用事件，多播器时可用， 这个多播器后面应该有介绍， 应该是观察者模式，初期注册一些事件，然后后续通知到监听者, debug 发现 this.applicationListeners  size 为 0
	 ...
	 * 例如容器的ClassLoader和后处理器
	 * @param beanFactory the BeanFactory to configure
	 */
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		//  beanFactory 这玩意可以修改 bean 定义
		// Tell the internal bean factory to use the context's class loader etc.
		beanFactory.setBeanClassLoader(getClassLoader()); // 设置 Bean 的类加载器
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this)); // 添加Bean的后置处理器
		...
		if (beanFactory.containsBean(LOAD_TIME_WEAVER_BEAN_NAME)) {
			beanFactory.addBeanPostProcessor(new LoadTimeWeaverAwareProcessor(beanFactory)); // 添加Bean的后置处理器
	 ...
	 * respecting explicit order if given. 实例化并注册所有BeanPostProcessor Bean，并遵循显式顺序（如果有的话）。
	 * <p>Must be called before any instantiation of application beans. 必须在应用程序bean的任何实例化之前调用
	 */
	 ...
	 * 如果没定义则使用 SimpleApplicationEventMulticaster
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
	 ...
	 * 添加实现了 ApplicationListener 的bean 作为监听者
